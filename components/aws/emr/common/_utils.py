...
def wait_for_cluster(client, jobflow_id):
    ...
    while True:
        ...
        print(f"Cluster state: {state}, wait 15s for cluster to start up.")
        time.sleep(15)
...
def submit_spark_job(client, jobflow_id, job_name, jar_path, main_class, extra_args):
    ...
    print(f"Step Id {step_id} has been submitted")
    return step_id
...
def wait_for_job(client, jobflow_id, step_id):
    ...
    while True:
        ...
        print(f"Step state: {state}, wait 15s for step status update.")
        time.sleep(10)
